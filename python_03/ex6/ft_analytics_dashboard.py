def list_com(game_data):
    print("=== List Comprehension Examples ===")
    high_scorers = [p for p, d in game_data['players'].items()
                    if d['total_score'] > 2000]
    scores_doubled = [d['total_score'] * 2
                      for d in game_data['players'].values()]
    active_players = [p for p, d in game_data['players'].items()
...
def dict_com(game_data):
    print("\n=== Dict Comprehension Examples ===")
    player_scores = {p: d['total_score']
                     for p, d in game_data['players'].items()}
    score_categories = {
        'high': sum(1 for d in game_data['players'].values()
                    if d['total_score'] > 5000),
        'medium': sum(1 for d in game_data['players'].values()
                      if 2000 <= d['total_score'] <= 5000),
        'low': sum(1 for d in game_data['players'].values()
                   if d['total_score'] < 2000),
        }
    achievement_counts = {p: d['achievements_count']
                          for p, d in game_data['players'].items()}
...
def set_com(game_data):
    print("\n=== Set Comprehension Examples ===")
    unique_players = {p for p in game_data['players']}
    unique_achievements = {a for a in game_data['achievements']}
...
def combined_com(game_data):
    print("\n=== Combined Analysis ===")
    unique_players = {p for p in game_data['players']}
    unique_achievements = {a for a in game_data['achievements']}
    total_score = sum(p["total_score"] for p in game_data["players"].values())
    average_score = total_score / len(game_data["players"])
    top_performer = max(game_data['players'].items(),
    ...
    print(f"Top performer: {top_performer[0]}",
          f"({top_performer[1]['total_score']} points,",
          f"{top_performer[1]['achievements_count']} achievements)")
